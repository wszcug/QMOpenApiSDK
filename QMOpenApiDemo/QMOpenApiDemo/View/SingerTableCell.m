...
@implementation SingerTableCell
...
- (void) updateCellWithSinger:(QPSinger *)singer{
    self.songLabel.text = singer.name;
    self.singerLabel.text = [NSString stringWithFormat:@"专辑：%ld 歌曲：%ld",(long)singer.totalAlbums,singer.totalSongs];
    [self.albumCoverImageView sd_setImageWithURL:singer.smallCoverURL placeholderImage:[UIImage imageNamed:@"default_album"]];
}
